package com.jktb.jiaktb.service.impl;

import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import com.jktb.jiaktb.mapper.GroupUserMapper;
import com.jktb.jiaktb.domain.GroupUser;
import com.jktb.jiaktb.service.IGroupUserService;

/**
 * 团体信息Service业务层处理
 * 
 * @author jktb
 * @date 2021-03-17
 */
@Service
public class GroupUserServiceImpl implements IGroupUserService 
{
    @Autowired
    private GroupUserMapper groupUserMapper;

    /**
     * 查询团体信息
     * 
     * @param groupUserId 团体信息ID
     * @return 团体信息
     */
    @Override
    public GroupUser selectGroupUserById(Long groupUserId)
    {
        return groupUserMapper.selectGroupUserById(groupUserId);
    }

    /**
     * 查询团体信息列表
     * 
     * @param groupUser 团体信息
     * @return 团体信息
     */
    @Override
    public List<GroupUser> selectGroupUserList(GroupUser groupUser)
    {
        return groupUserMapper.selectGroupUserList(groupUser);
    }

    /**
     * 新增团体信息
     * 
     * @param groupUser 团体信息
     * @return 结果
     */
    @Override
    public int insertGroupUser(GroupUser groupUser)
    {
        return groupUserMapper.insertGroupUser(groupUser);
    }

    /**
     * 修改团体信息
     * 
     * @param groupUser 团体信息
     * @return 结果
     */
    @Override
    public int updateGroupUser(GroupUser groupUser)
    {
        return groupUserMapper.updateGroupUser(groupUser);
    }

    /**
     * 批量删除团体信息
     * 
     * @param groupUserIds 需要删除的团体信息ID
     * @return 结果
     */
    @Override
    public int deleteGroupUserByIds(Long[] groupUserIds)
    {
        return groupUserMapper.deleteGroupUserByIds(groupUserIds);
    }

    /**
     * 删除团体信息信息
     * 
     * @param groupUserId 团体信息ID
     * @return 结果
     */
    @Override
    public int deleteGroupUserById(Long groupUserId)
    {
        return groupUserMapper.deleteGroupUserById(groupUserId);
    }
}
